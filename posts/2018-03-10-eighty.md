...
Eighty is structured around the `Html` class, being an immutable chunk of HTML which knows how to render itself using its `Write(TextWriter)` method. `Html` defines a large collection of static methods (designed to be imported with `using static`), with names like `h1` and `p`, which compose bigger `Html` values out of smaller ones.
...
Of course, C# code will only ever look _a bit_ like HTML. Razor code looks much more like HTML than this! But Eighty has two big advantages which make it simpler and easier to use than Razor:
...
With Eighty, it's a question of building different `Html` values. You can use LINQ's high-level looping constructs:
...
Razor's two main tools for code reuse are _partial views_ and _helpers_. For the purposes of this article, they're roughly equivalent. Partial views can be returned directly from a controller but their model type is checked at runtime, whereas helpers define a set of parameters to be checked by the compiler.
...
This is the best of both worlds: the parameters are checked by the compiler as usual, but the returned `Html` value is a perfectly good standalone chunk of HTML, and can be rendered separately if necessary.
...
Perhaps this is a bit of an abuse of `IDisposable`, but this trick allows Twenty to generate HTML quickly and without generating any garbage while still making for a reasonable DSL. Compared to Eighty, Twenty does lose out on some flexibility and safety:

* You mustn't forget a `using` statement, or call `Dispose` more than once, or Twenty will output malformed HTML.
...
Eighty is available [on Nuget](https://www.nuget.org/packages/Eighty), along with some helpers to integrate Eighty with [ASP.NET](https://www.nuget.org/packages/Eighty.AspNet), [ASP.NET Core](https://www.nuget.org/packages/Eighty.AspNetCore) and [Nancy](https://www.nuget.org/packages/Eighty.Nancy). The code's all [on GitHub](https://github.com/benjamin-hodgson/Eighty).
